  ...
  return (
    <Card
      className={`bg-card/60 backdrop-blur-sm shadow-md hover:shadow-accent/50 transition-all duration-300 flex flex-col items-center justify-center text-center p-6 h-full ${
        disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:border-primary/70 border border-dashed border-input hover:border-primary/50'
      }`}
    ...
    >
      <CardContent className="flex flex-col items-center justify-center gap-3 p-0">
        <PlusCircle className={`h-12 w-12 ${disabled ? 'text-muted-foreground/50' : 'text-primary/70'}`} />
        <p className={`text-sm font-medium ${disabled ? 'text-muted-foreground/80' : 'text-card-foreground'}`}>
          Add New Model
        </p>
      </CardContent>
    </Card>
