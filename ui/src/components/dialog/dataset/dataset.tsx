    ...
    >
      <div className="flex items-center flex-grow truncate">
        ...
        <span className="truncate max-w-[calc(100%-4rem)]" title={item.name}>
          {" "}
          {/* Adjust max-width to account for thumbnail if needed, e.g., calc(100%-6rem) or adjust based on testing */}
          {item.name}{" "}
          {item.file
...
export function CreateDatasetDialog({
    ...
    if (dataset) {
      ...
      if (
        ...
        const initialFileItems = dataset.data.map((fileName, index) => ({
          ...
          name: fileName as string,
          file: undefined,
          thumbnail: undefined, // Thumbnails are not stored on backend, will be generated if needed by UI
        }));
        setFileItems(initialFileItems);
      } else if (
        dataset.type === "list" &&
        ...
        // Reset for other types if necessary, e.g. if type is image but data is invalid
        if (dataset.type === "image" || dataset.type === "csv") {
           setFileItems([]);
        }
      }
    } else {
      // Reset form when dataset is undefined (creating new)
      resetForm();
    }
  }, [isOpen, dataset]);
  ...
  const validate = (): boolean => {
    ...
    if (type === "csv" && dataset === undefined && fileItems.length === 0) {
      ...
      isValid = false;
    } else if (type === "image" && dataset === undefined && fileItems.length === 0) {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      if (type === "csv") {
        ...
        // Potentially handle other types or default behavior
      ...
      if (errorMessage) {
        ...
        if (type === "image") {
                ...
                reader.onload = (e) => {
                  ...
                  img.onload = () => {
                    ...
                    if (ctx) {
                      ...
                      item.thumbnail = canvas.toDataURL(item.file?.type || "image/png");
                    } else {
                      item.thumbnail = undefined; // Failed to get context
                    }
                    resolve(item);
                  };
                  img.onerror = () => {
                    item.thumbnail = undefined; // Image failed to load
                    resolve(item);
                  };
                  img.src = e.target?.result as string;
                };
                reader.onerror = () => {
                  item.thumbnail = undefined; // FileReader failed
                  resolve(item);
                };
                reader.readAsDataURL(item.file);
              });
            }
            return Promise.resolve(item); // Not an image file or no file present
          });
          ...
          if (type === "csv" && filesError === "Please select at least one CSV file") {
            setFilesError("");
          }
        }
      }
