...
vi.mock("../model-selector", () => ({
  ModelSelector: vi.fn(({ selectModel, value, id, className }) => (
    <input
      ...
      value={value || ""}
      onChange={(e) => {
        if (selectModel) selectModel(e.target.value);
      }}
...
const mockGenerateOptions = actions.generateOptions as MockedFunction<
  typeof actions.generateOptions
>;
...
describe("GenerateOptionsDialog", () => {
  ...
  describe("User Interactions and Action Calls", () => {
    ...
    it("calls generateOptions, onGenerationComplete, onClose, and shows success toast on successful generation", async () => {
      const mockSelectedModel = "z";
      const mockPrompt =
        "Generate some good options for MyDataset (A test dataset).";
      mockGenerateOptions.mockResolvedValueOnce([
        "Generated Option X",
        "Generated Option Y",
      ]);
