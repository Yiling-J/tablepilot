...
describe("Workflow Run", () => {
  ...
  it("should ask for variables and run workflow", async () => {
    ...
    const dialogStartButton = within(variableDialog).getByRole("button", {
      name: "Start",
    });
  ...
  it("should run workflow to completion and update UI", async () => {
    ...
    mockedRunWorkflow.mockImplementation(
      async (
        ...
        eventCallback(
          JSON.stringify({ type: "MESSAGE", data: "Workflow starting..." }),
        );
    ...
    if (!mainStartButton)
      throw new Error("Main Start button not found after waitFor");
    ...
    await user.click(v2SelectTrigger);
    await user.click(await screen.findByText("c"));
    const dialogStartButton = within(
      screen.getByRole("dialog", { name: "Input Variables" }),
    ...
    await waitFor(
      () => {
        ...
        expect(
          within(stepElement.closest("li")!).getByText("Running"),
        ).toBeInTheDocument();
      },
      { timeout: 200 },
    );
    ...
    await waitFor(async () => {
      ...
      expect(
        within(step0Row.closest("li")!).getByText("Completed"),
      ).toBeInTheDocument();
      const step1Row = await screen.findByText(workflowSteps[1]);
      expect(
        within(step1Row.closest("li")!).getByText("Running"),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(async () => {
      const step1Row = await screen.findByText(workflowSteps[1]);
      expect(
        within(step1Row.closest("li")!).getByText("Completed"),
      ).toBeInTheDocument();
      const step2Row = await screen.findByText(workflowSteps[2]);
      expect(
        within(step2Row.closest("li")!).getByText("Running"),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(async () => {
      const step2Row = await screen.findByText(workflowSteps[2]);
      expect(
        within(step2Row.closest("li")!).getByText("Completed"),
      ).toBeInTheDocument();
    });
    ...
    for (const stepName of workflowSteps) {
      ...
      expect(
        within(stepRow.closest("li")!).getByText("Completed"),
      ).toBeInTheDocument();
    }
    ...
    await waitFor(() => {
      const completedIcons = document.querySelectorAll(
  ...
  it("should handle workflow error and update UI accordingly", async () => {
    ...
    await waitFor(async () => {
      ...
      expect(
        step0ListItem.querySelector("svg.text-green-500"),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(async () => {
      ...
      expect(
        step1ListItem.querySelector("svg.text-red-500"),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(async () => {
      ...
      expect(
        step2ListItem.querySelector("svg.text-gray-400"),
      ).toBeInTheDocument();
    });
  ...
  it("should abort workflow when Stop button is clicked", async () => {
    ...
    mockedRunWorkflow.mockImplementation(
      async (
        ...
        if (signal.aborted) {
          eventCallback(
            JSON.stringify({ type: "MESSAGE", data: userAbortMessage }),
          );
          eventCallback("[DONE]");
          return;
        }
    ...
    expect(
      within(step1Row.closest("li")!).queryByText("Failed"),
    ).not.toBeInTheDocument();
    const step2Row = await screen.findByText(workflowSteps[2]);
    expect(
      within(step2Row.closest("li")!).getByText("Pending"),
    ).toBeInTheDocument();

    expect(
      screen.queryByText("Workflow completed successfully!"),
    ).not.toBeInTheDocument();
  });
});
