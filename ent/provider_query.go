...
func (pq *ProviderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Provider, error) {
	...
	if query := pq.withModels; query != nil {
		if err := pq.loadModels(ctx, query, nodes,
			func(n *Provider) { n.Edges.Models = []*Model{} },
			func(n *Provider, e *Model) { n.Edges.Models = append(n.Edges.Models, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProviderQuery) loadModels(ctx context.Context, query *ModelQuery, nodes []*Provider, init func(*Provider), assign func(*Provider, *Model)) error {
	...
	query.Where(predicate.Model(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(provider.ModelsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.provider_models
		if fk == nil {
			return fmt.Errorf(`foreign-key "provider_models" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "provider_models" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
