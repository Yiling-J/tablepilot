...
func (mq *ModelQuery) QueryProvider() *ProviderQuery {
	query := (&ProviderClient{config: mq.config}).Query()
...
func (mq *ModelQuery) Clone() *ModelQuery {
	...
	return &ModelQuery{
		...
		withProvider: mq.withProvider.Clone(),
...
func (mq *ModelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Model, error) {
	...
	if query := mq.withProvider; query != nil {
		if err := mq.loadProvider(ctx, query, nodes, nil,
			func(n *Model, e *Provider) { n.Edges.Provider = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *ModelQuery) loadProvider(ctx context.Context, query *ProviderQuery, nodes []*Model, init func(*Model), assign func(*Model, *Provider)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Model)
	for i := range nodes {
		if nodes[i].provider_models == nil {
			continue
		}
		fk := *nodes[i].provider_models
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "provider_models" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
